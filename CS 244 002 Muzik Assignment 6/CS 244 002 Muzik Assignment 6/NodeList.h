...
/*===Rule of three ==================*/
	...
	template <class Elem> friend ostream& operator<<(ostream& os, const NodeList<Elem>&);
	/*===================================*/
...
template <class Elem>
ostream& operator<<(ostream& os, const NodeList<Elem>& NL) {
	os << "[";
	if (!NL.empty()) {
		for (typename NodeList<Elem>::Iterator curr = NL.begin(); curr != NL.end(); ++curr) {
			os << (*curr) << ",";
		}
	}
	os << "]";
