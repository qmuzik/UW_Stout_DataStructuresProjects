...
int main() {
	try {
		...
		while (userSelection != 8)
		{
			...
			case 0: system("cls");
				break;
			case 1: system("cls"); {
				NodeList<Vehicle>::Iterator current = mainInventory.beginIter();
				mainInventory.printAllRegisteredVehiclesRecursively(current);
			}
				  break;
			case 2: system("cls"); {
				NodeList<Vehicle>::Iterator current = mainInventory.beginIter();
				mainInventory.printAllAvailibleVehiclesRecursively(current);
			}
				  break;
			case 3: system("cls"); {
				NodeList<Vehicle>::Iterator current = mainInventory.beginIter();
				mainInventory.listOfReservedVehiclesRecursively(current);
			}
				  break;
			case 4: system("cls"); {
				int numOfSeats;
				...
				NodeList<Vehicle>::Iterator current = mainInventory.beginIter();
				mainInventory.checkAvailibilityRecursively(numOfSeats, current);
			}
				  break;
			case 5:  system("cls"); {
				int vehicleID;
	...
	// Catch for Input File not being read 
	catch (string e) {
		...
		if (e == 1) {
			...
			cout << "Exception: Invalid Vehicle ID\n" << "Code: " << e << endl;
			exit(4);
		}
	}
	return 0;
